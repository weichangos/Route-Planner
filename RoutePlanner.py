...
class RoutePlanner:
    ...
    def run(self):
        locs_dataframe = self.locs_to_pd_df()
        logging.info("Input data: \n %s", locs_dataframe)

        self.generate_graph(edge_probability=0.3)
        logging.info("Table for paths: \n %s", pd.DataFrame({'paths': list(nx.get_edge_attributes(self.G, "distance").keys()), 'distances': list(nx.get_edge_attributes(self.G, "distance").values())}))

        nodes_with_odd_degree = self.find_nodes_with_odd_degree()
        logging.info('Nodes with odd degree: \n %s', nodes_with_odd_degree)

        odd_node_pairs = self.compute_odd_node_pairs(nodes_with_odd_degree)
        logging.info("Potential odd node pairs: \n %s", odd_node_pairs)

        odd_node_pairs_shortest_paths = self.get_shortest_paths_distances(odd_node_pairs, 'distance')
        ...
        self.g_odd_complete = self.create_complete_graph(odd_node_pairs_shortest_paths, flip_weights=True)

        self.g_odd_complete_min_edges = self.find_com_min_edge(self.g_odd_complete)

        odd_matching = self.find_odd_matching(self.g_odd_complete)
        logging.info('Paths traversed twice, if not existed in original graph, shortest connecting path is automatically augmented: \n %s', odd_matching)

        self.g_aug = self.add_augmenting_path_to_graph(odd_matching)

        self.create_eulerian_circuit(starting_node='A')
        logging.info(self.euler_circuit)
